...
import { ChevronLeft, Utensils, History, Users } from "lucide-react";
...
export default function PastGroupHistory() {
    const { id } = useParams();
    // Since this page is specifically for "Past Groups" from Profile, back link is profile
        ...
        <div className="flex flex-col min-h-screen bg-slate-50 pb-20">
            <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 border-b border-slate-100">
                <div className="px-6 py-5 flex items-center gap-4">
                    <Link href={backLink} className="p-2 -ml-2 text-slate-400 hover:text-slate-800 transition-colors bg-slate-50 rounded-full">
                        <ChevronLeft size={20} strokeWidth={3} />
                    </Link>
                    <h2 className="text-2xl font-bold text-slate-800 tracking-tight">지난 밥조 식사 내역</h2>
                </div>
            </div>
            ...
            <div className="px-6 py-8 space-y-10">
                {isLoading ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-4">
                                {meals.map((meal) => (
                                    <div
                                        key={meal.id}
                                        className="p-6 rounded-[2.5rem] bg-white border border-slate-100 shadow-sm hover:shadow-lg transition-all space-y-5"
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <div className="text-right shrink-0">
                                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter block leading-none mb-1">총 결제</span>
                                                <span className="text-xl font-bold text-slate-600">₩{meal.total_amount.toLocaleString()}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-4 border-t border-slate-50 flex flex-wrap items-center justify-between gap-4">
                                            ...
                                            <div className="text-right shrink-0 bg-slate-800 text-white px-4 py-2 rounded-2xl shadow-lg shadow-slate-200">
                    ...
                    <div className="py-20 text-center text-slate-300 flex flex-col items-center gap-4">
                        ...
                        <p className="font-bold">기록된 식사가 없습니다.</p>
                    </div>
                )}
            </div>
        </div>
